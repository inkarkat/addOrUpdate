...
@test "error when no BLOCK passed" {
    run -2 updateBlock --marker test "$FILE"
...
@test "error when no marker passed" {
    run -2 updateBlock --block-text $'new\nblock' "$FILE"
...
@test "error when combining --ignore-nonexisting and --create-nonexisting" {
    run -2 updateBlock --ignore-nonexisting --create-nonexisting --marker test --block-text $'new\nblock' "$FILE"
    assert_line -n 0 'ERROR: Cannot combine --ignore-nonexisting and --create-nonexisting.'
    assert_line -n 1 -e '^Usage:'
}

@test "error on --create-nonexisting" {
    run -2 updateBlock --create-nonexisting --marker test --block-text $'new\nblock' "$FILE"
