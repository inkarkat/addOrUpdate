...
@test "message with multiple patches" {
    export MEMOIZEDECISION_CHOICE=y
    run -0 containedOrUpdateWithPatch --in-place "$PATCH" "$ALTERNATIVE_PATCH"
...
@test "message with single patch affecting multiple files" {
    ...
    cp -f "$EXISTING" "$MORE"
    export MEMOIZEDECISION_CHOICE=y
    run -0 containedOrUpdateWithPatch --in-place <(renamePatchTarget "$OTHER" "$PATCH"; renamePatchTarget "$MORE" "$PATCH")
...
@test "message with multiple patches affecting multiple files" {
    ...
    cp -f "$EXISTING" "$MORE"
    export MEMOIZEDECISION_CHOICE=y
    run -0 containedOrUpdateWithPatch --in-place <(renamePatchTarget "$OTHER" "$PATCH") "$REVERTED_PATCH" <(renamePatchTarget "$MORE" "$PATCH")
...
@test "--all message with single patch affecting multiple files" {
    ...
    cp -f "$EXISTING" "$MORE"
    export MEMOIZEDECISION_CHOICE=y
    run -0 containedOrUpdateWithPatch --all --in-place <(renamePatchTarget "$OTHER" "$PATCH"; renamePatchTarget "$MORE" "$PATCH")
    assert_output -e 'more\.txt, other\.txt do not yet contain .*\. Shall I apply it\?'
}
