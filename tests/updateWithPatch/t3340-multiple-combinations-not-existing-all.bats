...
@test "patching all with success and non-existing file returns 5" {
...
@test "patching all with non-existing and success returns 5" {
    run updateWithPatch --all --test-only doesNotExist.patch "$PATCH"
    [ $status -eq 5 ]
    [ "$output" = "patch: **** Can't open patch file doesNotExist.patch : No such file or directory" ]
}

@test "patching all with non-existing and non-applicable returns 5" {
...
@test "patching all with non-applicable and non-existing returns 5" {
...
@test "patching all with no-mod and non-existing returns 5" {
...
@test "patching all with non-existing and no-mod returns 5" {
