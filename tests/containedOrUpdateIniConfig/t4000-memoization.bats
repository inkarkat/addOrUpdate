...
@test "asks again on confirm each" {
    ...
    run -0 containedOrUpdateIniConfig --memoize-group containedOrUpdateIniConfig --in-place --section default --key foo --value new "$FILE"
...
@test "does not recall another file on yes" {
    ...
    run -0 containedOrUpdateIniConfig --memoize-group containedOrUpdateIniConfig --in-place --section default --key foo --value new "$FILE2"
