...
@test "does not modify the file if the file already contains the assignment" {
    run containedOrUpdateIniConfig --in-place --section default --key foo --value bar "$FILE"
    diff -y "$INPUT" "$FILE"
}

@test "returns 99 and error message if the file already contains the assignment" {
    run -99 containedOrUpdateIniConfig --in-place --section default --key foo --value bar "$FILE"
...
@test "returns 99 and error message mentioning the name if the file already contains the assignment" {
    NAME="My test file"
    run -99 containedOrUpdateIniConfig --in-place --name "$NAME" --section default --key foo --value bar "$FILE"
...
@test "returns 99 and no error message with an empty one passed if the file already contains the assignment" {
    run -99 containedOrUpdateIniConfig --up-to-date-message '' --in-place --section default --key foo --value bar "$FILE"
    assert_output ''
}
...
@test "returns 99 and a custom passed message if the file already contains the assignment" {
    ...
    run -99 containedOrUpdateIniConfig --up-to-date-message "$MESSAGE" --in-place --section default --key foo --value bar "$FILE"
    assert_output "$MESSAGE"
}

@test "returns 4 if none of the passed files exist" {
    run -4 containedOrUpdateIniConfig --in-place --section default --key foo --value bar "$NONE" "$NONE2"
    assert_output ''
}
