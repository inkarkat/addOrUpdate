...
@test "update in all existing files skips nonexisting files" {
    run -0 updateIniConfig --all --in-place --section default --key foo --value new "$NONE" "$FILE" "$NONE2" "$FILE2"
    assert_output ''
...
@test "all nonexisting all files returns 4" {
    run -4 updateIniConfig --all --in-place --section default --key foo --value new "$NONE" "$NONE2"
    assert_output ''
    assert_not_exists "$NONE"
    assert_not_exists "$NONE2"
}
