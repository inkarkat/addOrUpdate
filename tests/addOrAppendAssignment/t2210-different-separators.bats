...
@test "update with comma separator appends to existing values" {
    ASSIGNMENT_QUOTE= ASSIGNMENT_SEPARATOR=, run addOrAppendAssignment --lhs foz --rhs add "$FILE"
    [ $status -eq 0 ]
    [ "$output" = "foo=\"bar\"
foo='bar'
foo=/initial/value/
...
@test "update with forward slash quotes and same separator appends to existing values" {
    ASSIGNMENT_QUOTE=/ ASSIGNMENT_SEPARATOR=/ run addOrAppendAssignment --lhs foo --rhs add "$FILE"
    [ $status -eq 0 ]
    [ "$output" = "foo=\"bar\"
foo='bar'
