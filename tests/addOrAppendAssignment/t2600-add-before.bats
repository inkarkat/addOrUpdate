...
@test "update with nonexisting assignment inserts on the passed line" {
    run -0 addOrAppendAssignment --lhs new --rhs add --add-before 4 "$FILE"
    assert_output - <<'EOF'
sing/e="wha\ever"
foo="bar"
foo="hoo bar baz"
...
@test "update with nonexisting assignment inserts on the passed ADDRESS" {
    run -0 addOrAppendAssignment --lhs new --rhs add --add-before '/^#/' "$FILE"
    assert_output - <<'EOF'
sing/e="wha\ever"
foo="bar"
foo="hoo bar baz"
...
@test "update with nonexisting assignment inserts on the first match of ADDRESS only" {
    run -0 addOrAppendAssignment --lhs new --rhs add --add-before '/^foo=/' "$FILE"
    assert_output - <<'EOF'
sing/e="wha\ever"
new="add"
foo="bar"
...
@test "update with existing assignment one before the passed line keeps contents and returns 1" {
    run -0 addOrAppendAssignment --lhs foo --rhs bar --add-before 1 "$FILE"
    assert_output - <<'EOF'
foo="bar"
sing/e="wha\ever"
foo="bar"
