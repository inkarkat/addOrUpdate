...
@test "updating existing file in not-writable dir returns 5" {
    run -5 addOrUpdateIniConfig --in-place --section default --key foo --value new "$IMMUTABLE_DIRSPEC"
...
@test "creating a nonexisting file in a non-writable directory returns 5" {
    ...
    run -5 addOrUpdateIniConfig --create-nonexisting --in-place --section default --key foo --value new "$IMMUTABLE_NEW"
    assert_equal ${#lines[@]} 1
...
@test "creating a nonexisting file in a nonexisting directory returns 5" {
    ...
    run -5 addOrUpdateIniConfig --create-nonexisting --in-place --section default --key foo --value new "$NONEXISTING"
    assert_equal ${#lines[@]} 1
