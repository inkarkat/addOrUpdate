...
@test "update with existing last configuration replaces config" {
    run -0 addOrUpdateIniConfig --section 'last one' --key foo --value new "$FILE"
    assert_output - <<'EOF'
[global]
foo=none
bar=hey
...
@test "update with existing configuration in section containing forward and backslash replaces config" {
    run -0 addOrUpdateIniConfig --section 'spec/ial\one' --key foo --value new "$FILE"
    assert_output - <<'EOF'
[global]
foo=none
bar=hey
...
@test "update with existing default configuration replaces first occurring config" {
    run -0 addOrUpdateIniConfig --section 'default' --key foo --value new "$FILE"
    assert_output - <<'EOF'
[global]
foo=none
bar=hey
...
@test "update with existing global configuration in first section replaces first occurring config" {
    run -0 addOrUpdateIniConfig --section 'global' --key foo --value new "$FILE"
    assert_output - <<'EOF'
[global]
foo=new
bar=hey
