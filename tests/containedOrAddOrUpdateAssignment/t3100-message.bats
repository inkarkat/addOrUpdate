...
@test "message with multiple files" {
    init
    export MEMOIZEDECISION_CHOICE=y
    run -0 containedOrAddOrUpdateAssignment --in-place --lhs foo --rhs new "$FILE" "$FILE2" "$FILE3"
...
@test "--all message with single file" {
    init
    export MEMOIZEDECISION_CHOICE=y
    run -0 containedOrAddOrUpdateAssignment --all --in-place --lhs foo --rhs new "$FILE"
    assert_output -p "does not yet contain 'foo=new'. Shall I update it?"
}
...
@test "--all message with multiple files" {
    init
    export MEMOIZEDECISION_CHOICE=y
    run -0 containedOrAddOrUpdateAssignment --all --in-place --lhs foo --rhs new "$FILE" "$FILE2" "$FILE3"
