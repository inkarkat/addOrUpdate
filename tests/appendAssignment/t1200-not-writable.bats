...
@test "updating existing file in not-writable dir returns 5" {
    run appendAssignment --in-place --lhs foo --rhs new "$IMMUTABLE_DIR_FILESPEC"
    [ $status -eq 5 ]
    [ "${#lines[@]}" -eq 1 ]
    [[ "$output" =~ ^sed: ]]
}
...
@test "creating a nonexisting file in a non-writable directory returns 5" {
    ...
    [ ! -e "$IMMUTABLE_NEW" ]
    run appendAssignment --create-nonexisting --in-place --lhs foo --rhs new "$IMMUTABLE_NEW"
    [ $status -eq 5 ]
    [ "${#lines[@]}" -eq 1 ]
...
@test "creating a nonexisting file in a nonexisting directory returns 5" {
    ...
    run appendAssignment --create-nonexisting --in-place --lhs foo --rhs new "$NONEXISTING"
    [ $status -eq 5 ]
    [ "${#lines[@]}" -eq 1 ]
