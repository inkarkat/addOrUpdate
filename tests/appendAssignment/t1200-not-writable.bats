...
@test "updating existing file in not-writable dir returns 5" {
    run -5 appendAssignment --in-place --lhs foo --rhs new "$IMMUTABLE_DIR_FILESPEC"
...
@test "creating a nonexisting file in a non-writable directory returns 5" {
    ...
    run -5 appendAssignment --create-nonexisting --in-place --lhs foo --rhs new "$IMMUTABLE_NEW"
    assert_equal ${#lines[@]} 1
...
@test "creating a nonexisting file in a nonexisting directory returns 5" {
    ...
    run -5 appendAssignment --create-nonexisting --in-place --lhs foo --rhs new "$NONEXISTING"
    assert_equal ${#lines[@]} 1
