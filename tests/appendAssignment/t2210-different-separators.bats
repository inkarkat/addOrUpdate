...
@test "update with comma separator appends to existing values" {
    ASSIGNMENT_QUOTE= ASSIGNMENT_SEPARATOR=, run -0 appendAssignment --lhs foz --rhs add "$FILE"
    assert_output - <<'EOF'
foo="bar"
foo='bar'
foo=/initial/value/
...
@test "update with forward slash quotes and same separator appends to existing values" {
    ASSIGNMENT_QUOTE=/ ASSIGNMENT_SEPARATOR=/ run -0 appendAssignment --lhs foo --rhs add "$FILE"
    assert_output - <<'EOF'
foo="bar"
foo='bar'
